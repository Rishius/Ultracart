...
class UltraCart
{
  ...
  public $ucServiceUrl = 'https://secure.ultracart.com/axis/services/CheckoutAPIV1?wsdl';
  ...
  public function __construct($merchantId)
  {
    ...
    if (!isset($_COOKIE["cartId"])) {
      $cart = $this->soapClient->createCart($merchantId);
      if (!is_null($cart)) {
        $this->cart = $cart;
      ...
      $cart = $this->soapClient->getCart($merchantId, $cartId);
  ...
  {
    $result = new CartOperationResult();
    $cart = $this->soapClient->logoutCustomer($this->merchantId, $this->cart);
    if (!is_null($cart)) {
      $this->cart = $cart;
      $this->hasCart = true;
    }
  ...
  {
    $result = new CartOperationResult();
    $cart = $this->soapClient->removeCoupon($this->merchantId, $this->cart, $couponCode);
    if (!is_null($cart)) {
      $this->cart = $cart;
      $this->hasCart = true;
    }
  ...
  {
    $result = new CartOperationResult();
    $cart = $this->soapClient->updateCart($this->merchantId, $this->cart);
    if (!is_null($cart)) {
      $this->cart = $cart;
    } else {
      $result->wasSuccessful = false;
  ...
  public function validate($checks = array())
  {
    ...
    $errors = null;
    if (count($checks) > 0) {
      $errors = $this->soapClient->validate($this->merchantId, $this->cart, $checks);
    } else {
      $errors = $this->soapClient->validate($this->merchantId, $this->cart);
    }
  ...
  {
    return $this->soapClient->estimateShipping($this->merchantId, $this->cart);
  }
  ...
  public function checkoutGoogle()
  {
    ...
    if ($this->hasCart) {
      $this->updateCart();
      $handOff = $this->soapClient->googleCheckoutHandoff($this->merchantId, $this->cart, ucCurrentUrl(), 'checkout_err[]');
      if (is_null($handOff)) {
        $result->wasSuccessful = false;
        array_push($result->errorMessages, 'Google Checkout is unavailable at this time.  Sorry.');
      } else {
        if (property_exists($handOff, 'errors') && count($handOff->errors) > 0) {
          $result->wasSuccessful = false;
          foreach ($handOff->errors as $err) {
            array_push($result->errorMessages, $err);
          }
        } else {
          $result->wasSuccessful = true;
          $result->returnValue = $handOff->redirectToUrl;
        }
      }
    }
    return $result;
  ...
  public function checkoutPayPal()
  {
    ...
    if ($this->hasCart) {
      $this->updateCart();
      $handOff = $this->soapClient->paypalHandoff($this->merchantId, $this->cart, ucCurrentUrl(), 'checkout_err[]');
      if (is_null($handOff)) {
        $result->wasSuccessful = false;
  ...
  public function checkout()
  {
    ...
    if ($this->hasCart) {
      $this->updateCart();
      $handOff = $this->soapClient->checkoutHandoff($this->merchantId, $this->cart, ucCurrentUrl(), 'checkout_err[]');
      if (is_null($handOff)) {
        $result->wasSuccessful = false;
