...
class UltraCart
{
    ...
    public function __construct($merchantId, $login = false, $password = false)
    {
        error_log("init uc with this cookie: " . $_COOKIE["cartId"]);
        ...
        if (isset($_COOKIE["cartId"])) {
            ...
            $cart = $this->soapClient->getCart($merchantId, $_COOKIE["cartId"])->cart;
        }
        if (is_null($cart)) {
            error_log('calling createCart');
            /** @noinspection PhpUndefinedMethodInspection */
            $cart = $this->soapClient->createCart($merchantId)->cart;
        }
    ...
    {
        if (!is_null($cart)) {
            error_log("setting cart with cart id =" . $cart->cartId);
            $this->cart = $cart;
    ...
    {
        error_log("calling addItemsByItemIds() with this cart id: " . $this->cart->cartId);
        $result = new CartOperationResult();
        /** @noinspection PhpUndefinedMethodInspection */
        ...
        if (property_exists($cartChangeResult, 'cart')) {
            $cart = $cartChangeResult->cart;
            error_log("addItemsByItemIds() returned this cart id: " . $cart->cartId);
            $this->setCart($cart);
        }
    ...
    public function deleteItem($item)
    {
        ...
        if (!is_null($cartChangeResult)) {
            ...
            if (property_exists($cartChangeResult, 'cart')) {
                $this->setCart($cartChangeResult->cart);
            }
        }
    ...
    public function getRelatedItems()
    {
        $result = new CartOperationResult();
        /** @noinspection PhpUndefinedMethodInspection */
        $itemsResult = $this->soapClient->getRelatedItems($this->merchantId, $this->cart);
        if(!is_null($itemsResult)){
            if(property_exists($itemsResult, 'errors')){
                $result->wasSuccessful = false;
                foreach($itemsResult->errors as $err){
                    array_push($result->errorMessages, $err);
                }
            }
            if(property_exists($itemsResult, 'items')){
                $result->returnValue = $itemsResult->items;
            }
        }
    ...
    public function getRelatedItems2($itemId)
    {
        $result = new CartOperationResult();
        /** @noinspection PhpUndefinedMethodInspection */
        $itemsResult = $this->soapClient->getRelatedItems2($this->merchantId, $this->cart, $itemId);
        if(!is_null($itemsResult)){
            if(property_exists($itemsResult, 'errors')){
                $result->wasSuccessful = false;
                foreach($itemsResult->errors as $err){
    ...
    public function finalizeOrder()
    {
        ...
        if ($this->hasCart) {
            ...
            if (is_null($finalResult)) {
                ...
                if (property_exists($finalResult, 'errors') && count($finalResult->errors) > 0) {
                    $result->wasSuccessful = false;
                    ...
                    $result->wasSuccessful = true;
                    $result->returnValue = $finalResult;
                }
            }
        }
        return $result;
